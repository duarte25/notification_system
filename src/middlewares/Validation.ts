import mongoose, { Model, Document, mongo, Types } from "mongoose";
...
export class Validation {
  ...
  async validate(path: string, ...funcoes: any[]): Promise<Validation> {
...
export class ValidationFuncs {
  ...
  static mongoUUID = (opcoes: UniqueOptions = {}) => {
    return async (value: any, val: { path: string }): Promise<any> => {
      ...
      const isValidObjectId = Types.ObjectId.isValid(valorUUID);
